...
`
  ...
  // Load student files if it's a student booking
  if (isStudentBooking) {
    loadStudentFiles(booking.id);
  } else {
...
function updateModalButtons(booking, isStudentBooking) {
  const downloadBtn = document.getElementById("downloadBtn");
  const approveBtn = document.getElementById("approveBtn");
  const declineBtn = document.getElementById("declineBtn");
  ...
  // For student bookings, hide download button
  if (isStudentBooking) {
...
async function openUploadModal(bookingId) {
  ...
  // **NEW: Show admin items, hide student items**
  showAdminUploadItems();
  hideStudentUploadItems();

  // **NEW: Set total count for user bookings**
  document.getElementById("totalCount").textContent = "7";
...
async function saveUploadedFiles() {
  // Check if this is a student booking by checking which file types are visible
  const studentFileTypes = [
    "permission_letter",
    "request_letter",
    "approval_letter",
  ];
  const isStudentUpload = studentFileTypes.some((type) => {
    const item = document.querySelector(`[data-file-type="${type}"]`);
    return item && item.style.display !== "none";
  });

  if (isStudentUpload) {
    return await saveStudentUploadedFiles();
  } else {
...
// Save uploaded files for student bookings
async function saveStudentUploadedFiles() {
  const newFiles = Object.keys(uploadedFiles).filter(
    (fileType) => !uploadedFiles[fileType].isExisting
  );
  ...
  try {
    ...
    // Student files use array format with proper indices
