...
public function getEquipment()
{
    try {
        ...
        return $this->response->setJSON([
            'success' => true,
            'equipment' => $equipment,
            'filtered_by_date' => !empty($eventDate),
...
if (!empty($equipment)) {
    ...
    public function deleteFacility($facilityId)
    {
        try {
            ...
            $bookingCount = $this->bookingModel->where('facility_id', $facilityId)->countAllResults();
            if ($bookingCount > 0) {
                return $this->response->setJSON([
                    'success' => false,
                    'message' => 'Cannot delete facility with existing bookings. Consider marking it as inactive instead.'
                ])->setStatusCode(400);
            }
