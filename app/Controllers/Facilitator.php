...
class Facilitator extends Controller
{
    ...
    public function index()
    {
        ...
        if ($this->session->get('role') !== 'facilitator') {
            return redirect()->to('/unauthorized')->with('error', 'Access denied. Facilitator role required.');
        }
    ...
    public function getCompletedEvents()
    {
        try {
            // Get events that are:
            // 1. Status is 'scheduled' (not yet inspected)
            // 2. Past the event date
            // 3. Join with bookings to ensure booking was confirmed
            ...
            log_message('info', 'Facilitator - Found ' . count($processed) . ' completed events');
    ...
    public function generateEquipmentReport($eventId)
    {
        try {
            ...
            // Row 27 is where plan equipment starts
            $planRow = 27;
            ...
            if ($planEquipmentCount > 0) {
                ...
                foreach ($planEquipment as $index => $equip) {
                    ...
                    // Column B: Quantity
                    $sheet->setCellValue('B' . $currentRow, $equip['quantity']);
                    ...
                    // Set status in column C (or next available column for status)
                    $sheet->setCellValue('C' . $currentRow, $statusText);
                }
