...
<html lang="en">
  <head>
    ...
    <title>University Gymnasium - CSPC Digital Booking System</title>
    <link
    ...
    <section class="hero-section">
      <div class="container">
        <div class="hero-content">
          ...
          <h1>University Gymnasium</h1>
          <p>
            World-class multi-purpose sports facility equipped with professional lighting systems, 
            advanced sound equipment, and comprehensive event solutions for all your athletic and entertainment needs.
          </p>
        </div>
      </div>
    </section>
    ...
    <section class="facility-details-section">
      <div class="container">
        ...
        <div class="facility-hero">
          ...
          <h2>Premium Sports & Events Venue</h2>
          <p>
            Our University Gymnasium offers a state-of-the-art facility perfect for athletic events, 
            concerts, conferences, and large gatherings. With professional-grade equipment and 
            flexible setup options, we can accommodate events of any scale.
          </p>
        </div>
        ...
        <div class="row">
          ...
          <div class="col-lg-3 col-md-6 mb-4">
            <div class="footer-section">
              ...
              <a href="/sports">Sports Complex</a>
            </div>
          </div>
          <div class="col-lg-3 col-md-6 mb-4">
            <div class="footer-section">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form id="bookingForm">
          ...
          <div class="alert alert-info mt-3" role="alert" style="background: #dbeafe; border-color: #3b82f6; color: #1e3a8a;">
            <i class="fas fa-info-circle me-2"></i>
            <strong style="color: #1e3a8a;">Payment Terms:</strong> A 50% deposit is required to confirm your booking. Full payment is due 24 hours before the event date.
          </div>
        </form>
      </div>
...
let currentFacility = 'gymnasium'; // Changed from 'avr-engineering'
const MAINTENANCE_FEE = 2000;
const HOURLY_RATE = 1500; // Changed from 500 for gymnasium
...
async function loadFacilityData() {
  try {
    const response = await fetch("<?= base_url('api/facilities/data/gymnasium') ?>");
    const data = await response.json();
    facilityData = data.facility || data; // Extract facility object from response
...
// Render packages dynamically from facility data
function renderPackages() {
  ...
  facilityData.plans.forEach((plan, index) => {
    ...
    const isMostPopular = index === 0; // Mark first plan as most popular
...
async function loadAddonsData() {
  try {
    ...
    // Render charges after loading addons
    renderCharges();
  } catch (error) {
...
// Render additional charges dynamically from addons data
  ...
  addonsData.forEach((addon) => {
    html += `
      <div class="charge-item">
        ...
        <span class="charge-price">₱${parseFloat(addon.price).toLocaleString()}</span>
      </div>
    `;
  });
...
async function loadEquipmentData() {
  try {
    // Use public equipment endpoint (api/equipment) instead of protected booking endpoint
    const response = await fetch("<?= base_url('api/equipment') ?>");
    
    if (!response.ok) {
      console.error(`Equipment API returned status ${response.status}`);
    ...
    if (!result.success) {
      console.warn("Equipment API returned success: false", result.message);
      equipmentData = [];
      return;
    }

    // Filter equipment - ONLY furniture/logistics that are rentable with rate > 0
    ...
    console.log("Filtered equipment count:", equipmentData.length);
...
async function checkEquipmentAvailabilityOnDate(eventDate) {
  try {
    ...
    if (result.success && result.equipment) {
      // Update equipment availability using the full equipment array from API
      equipmentData = result.equipment
        .filter((equipment) => {
          const isRentable =
            equipment.is_rentable == 1 ||
      ...
      console.log("Equipment availability on " + eventDate + ":", equipmentData);
      populateEquipment(); // Re-render with updated availability
      updateCostSummary();
    } else {
      console.warn("Could not get equipment availability for date:", eventDate);
      // Keep existing availability if API fails
    }
  } catch (error) {
...
function openBookingModal(packageName) {
  ...
  if (!selectedPlan) {
    ...
    console.error('Plan not found:', packageName);
    return;
  }
  ...
  // Update dynamic hourly rate
  const additionalHoursRate = facilityData.additional_hours_rate || 1500;
  ...
  // Hide equipment section until date is selected
  ...
  if (eventDate) {
    ...
    try {
      ...
      if (conflictResult.hasConflict) {
        ...
        // Show placeholder instead of equipment
...
function showAccessDeniedModal(packageName) {
      ...
      <div class="equipment-item mb-3">
        ...
        ${!isAvailable
          ...
          : `<input type="number" class="form-control quantity-input" id="qty-${equipment.id}" 
                   min="0" max="${equipment.available}" value="0" onchange="updateEquipment('${equipment.id}')" style="border-radius: 8px; border: 2px solid #e2e8f0;">
...
function updateCostSummary() {
  ...
  // Use dynamic hourly rate from facility data
  const additionalHoursRate = facilityData.additional_hours_rate || 1500;
...
async function submitBooking() {
  ...
  const userModel = new Map(); // Placeholder for user data
  
  const formData = {
    facility_key: currentFacility,
    plan_id: selectedPlan.id,
    client_name: document.getElementById("organization")?.value || "Guest",
  ...
  console.log("Total duration calculated:", totalDuration, "hours");
  console.log("Event date:", formData.event_date);
  console.log("Event time:", formData.event_time);
