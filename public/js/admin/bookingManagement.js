  ...
  detailsContainer.innerHTML = `
    ...
    <div class="detail-section">
        ...
        <div class="info-message" style="margin-top: 15px; padding: 10px; background: #e3f2fd; border-left: 4px solid #2196f3; border-radius: 4px;">
            <strong>ℹ️ Student Booking:</strong> This is a student organization booking. No payment required.
        </div>
    </div>
    `
    }
...
`
  ...
  // Load free booking files if it's a student or faculty booking
  const isFreeBooking = isStudentBooking || isFacultyBooking;
  if (isFreeBooking) {
...
function updateModalButtons(booking, isStudentBooking, isFacultyBooking) {
  const downloadBtn = document.getElementById("downloadBtn");
  const approveBtn = document.getElementById("approveBtn");
  const declineBtn = document.getElementById("declineBtn");
  ...
  // For student and faculty bookings, hide download button
  const isFreeBooking = isStudentBooking || isFacultyBooking;
  if (isFreeBooking) {
...
function downloadSingleFile(fileType) {
  ...
  if (fileType === "evaluation") {
    downloadFacultyEvaluation(currentBookingId);
    return;
  }
...
async function openUploadModal(bookingId) {
  ...
  // **Check booking type: show student/faculty files or admin files**
  const isFreeBooking =
    booking.booking_type === "student" || booking.booking_type === "faculty";

  if (isFreeBooking) {
    // Show student file items for free bookings
    hideAdminUploadItems();
    showStudentUploadItems();
    document.getElementById("totalCount").textContent = "3"; // 3 files for student/faculty
  } else {
...
async function saveUploadedFiles() {
  // Check if this is a student/faculty booking by checking which file types are visible
...
// Save uploaded files for free bookings (student and faculty)
async function saveFreeBookingUploadedFiles() {
  ...
  try {
    ...
    // Free booking files (student and faculty) use array format with proper indices
...
async function downloadFacultyEvaluation(bookingId) {
  if (!bookingId) {
    console.error("[Faculty Evaluation] No booking ID provided");
    alert("❌ Error: No booking ID provided");
    return;
  }
  ...
  try {
    console.log(
      `[Faculty Evaluation] Starting download for booking #${bookingId}`
    );

    // First, check if there's a submitted survey evaluation file
    console.log(
      `[Faculty Evaluation] Fetching survey files from: /api/survey-files/${bookingId}`
    );
    ...
    console.log(
      `[Faculty Evaluation] Survey files response status: ${surveyFilesResponse.status}`
    );
    console.log(
      `[Faculty Evaluation] Survey files response OK: ${surveyFilesResponse.ok}`
    );

    if (surveyFilesResponse.ok) {
      const surveyResult = await surveyFilesResponse.json();
      console.log(`[Faculty Evaluation] Survey result:`, surveyResult);
      ...
      if (
        ...
        console.log(
          `[Faculty Evaluation] Found survey evaluation file:`,
          surveyFile
        );
        console.log(`[Faculty Evaluation] File name: ${surveyFile.name}`);
        console.log(`[Faculty Evaluation] File URL: ${surveyFile.url}`);
        console.log(`[Faculty Evaluation] File size: ${surveyFile.size} bytes`);
        ...
        try {
          ...
          console.log(
            `[Faculty Evaluation] File accessibility check status: ${fileCheckResponse.status}`
          );

          if (!fileCheckResponse.ok) {
            console.warn(
              `[Faculty Evaluation] File may not be accessible (HTTP ${fileCheckResponse.status})`
            );
          }
        } catch (checkError) {
          console.warn(
            `[Faculty Evaluation] Could not verify file accessibility:`,
            checkError
          );
        }

        // Download the survey file
        console.log(`[Faculty Evaluation] Creating download link...`);
        const a = document.createElement("a");
        ...
        console.log(
          `[Faculty Evaluation] Download element created with href: ${a.href}`
        );
        console.log(`[Faculty Evaluation] Download filename: ${a.download}`);

        a.click();

        console.log(`[Faculty Evaluation] Download click triggered`);

        // Remove after a delay to allow download to start
        setTimeout(() => {
          document.body.removeChild(a);
          console.log(`[Faculty Evaluation] Download element removed from DOM`);
        }, 100);

        console.log(
          `[Faculty Evaluation] Survey evaluation file download completed: ${surveyFile.name}`
        );
        return;
      } else {
        console.log(
          `[Faculty Evaluation] No survey files found in response. Success: ${
            surveyResult.success
      ...
      console.warn(
        `[Faculty Evaluation] Survey files API returned status ${surveyFilesResponse.status}`
      );
      const errorText = await surveyFilesResponse.text();
      console.warn(`[Faculty Evaluation] Response body:`, errorText);
    }
  } catch (error) {
    console.error(`[Faculty Evaluation] Error fetching survey file:`, error);
    console.error(`[Faculty Evaluation] Error message: ${error.message}`);
    console.error(`[Faculty Evaluation] Error stack: ${error.stack}`);
    console.log(`[Faculty Evaluation] Falling back to template generation...`);
  }

  // If no survey file exists, generate blank template
  console.log(
    `[Faculty Evaluation] No survey file found or error occurred, generating blank template`
  );
  alert(
    `Generating Faculty Evaluation Form for booking BK${String(
      bookingId
    ).padStart(3, "0")}...`
  );

  const downloadUrl = `${API_BASE_URL}/${bookingId}/faculty-evaluation`;
  console.log(`[Faculty Evaluation] Template download URL: ${downloadUrl}`);
  ...
  try {
    ...
    console.log(
      `[Faculty Evaluation] Template download element created with href: ${a.href}`
    );

    a.click();

    console.log(`[Faculty Evaluation] Template download click triggered`);

    setTimeout(() => {
      document.body.removeChild(a);
      console.log(
        `[Faculty Evaluation] Template download element removed from DOM`
      );
    }, 100);

    console.log(
      `[Faculty Evaluation] Faculty Evaluation template download initiated for booking: ${bookingId}`
    );
  } catch (error) {
    console.error(
      `[Faculty Evaluation] Error initiating template download:`,
      error
    );
    console.error(`[Faculty Evaluation] Error message: ${error.message}`);
    alert(`❌ Failed to download Faculty Evaluation Form: ${error.message}`);
  }
}
...
// Load free booking files (student and faculty)
...
// Download free booking file (student or faculty)
    ...
    if (data.success && data.files && data.files.length > 0) {
      ...
      container.innerHTML = `
        <p style="color: var(--gray); font-style: italic; text-align: center; padding: 20px;">
          ℹ️ No evaluation forms submitted yet. Forms will appear here after the client completes the survey.
        </p>
      `;
    }
  } catch (error) {
