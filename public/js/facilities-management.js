...
function renderFacilities(facilitiesData) {
    ...
    tbody.innerHTML = facilitiesData.map(facility => `
        <tr>
            ...
            <td class="facility-description" title="${facility.description || ''}">${facility.description || 'No description'}</td>
            <td>
                <span class="status-badge ${facility.is_active == 1 ? 'active' : 'inactive'}">
                    ${facility.is_active == 1 ? '✅ Active' : '❌ Inactive'}
                </span>
            </td>
            <td>
                <span class="maintenance-badge ${facility.is_maintenance == 1 ? 'yes' : 'no'}">
                    ${facility.is_maintenance == 1 ? '🔧 Yes' : '✓ No'}
                </span>
            </td>
            <td>
                <div class="action-btns">
                    <button class="btn-action btn-edit" onclick="editFacility(${facility.id})" title="Edit">
                        ✏️ Edit
                    </button>
                    <button class="btn-action btn-toggle" onclick="toggleActive(${facility.id}, ${facility.is_active})" title="Toggle Status">
                        ${facility.is_active == 1 ? '🚫' : '✅'}
                    </button>
                    <button class="btn-action btn-maintenance" onclick="toggleMaintenance(${facility.id}, ${facility.is_maintenance})" title="Toggle Maintenance">
                        ${facility.is_maintenance == 1 ? '✓' : '🔧'}
                    </button>
                    <button class="btn-action btn-delete" onclick="deleteFacility(${facility.id})" title="Delete">
                        🗑️
                    </button>
                </div>
            </td>
...
function updateStats(facilitiesData) {
    ...
    const active = facilitiesData.filter(f => f.is_active == 1 && f.is_maintenance == 0).length;
    const maintenance = facilitiesData.filter(f => f.is_maintenance == 1).length;
...
function filterFacilities() {
    const searchTerm = document.getElementById('searchFacilities').value.toLowerCase();
...
function editFacility(facilityId) {
    ...
    document.getElementById('extendedHourRate').value = facility.extended_hour_rate || 500;
    ...
    document.getElementById('isMaintenance').checked = facility.is_maintenance == 1;
    document.getElementById('facilityModal').style.display = 'block';
}
...
async function saveFacility() {
    ...
    const extendedHourRate = document.getElementById('extendedHourRate').value;
    ...
    const isMaintenance = document.getElementById('isMaintenance').checked ? 1 : 0;
    ...
    if (!extendedHourRate || parseFloat(extendedHourRate) < 0) {
        showToast('Valid extended hour rate is required', 'error');
        return;
    }
    ...
    const facilityData = {
        ...
        icon,
        extended_hour_rate: parseFloat(extendedHourRate),
        description,
        is_active: isActive,
    ...
    try {
        const url = facilityId ? `/api/facilities/update/${facilityId}` : '/api/facilities/create';
        const method = 'POST';
        ...
        if (result.success) {
            showToast(result.message || (facilityId ? 'Facility updated successfully' : 'Facility created successfully'), 'success');
            closeFacilityModal();
            loadFacilities();
        } else {
