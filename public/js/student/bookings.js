...
function checkSubmitButton() {
  ...
  // Check for already uploaded files (showing in previews)
  const hasPermissionPreview =
    document.getElementById("permissionPreview").style.display !== "none";
  const hasRequestPreview =
    document.getElementById("requestPreview").style.display !== "none";
  const hasApprovalPreview =
    document.getElementById("approvalPreview").style.display !== "none";

  // Enable submit if there are newly selected files OR existing uploads to manage
  document.getElementById("submitDocumentsBtn").disabled = !(
    hasPermission ||
    hasRequest ||
    hasApproval ||
    hasPermissionPreview ||
    hasRequestPreview ||
    hasApprovalPreview
  );
}
